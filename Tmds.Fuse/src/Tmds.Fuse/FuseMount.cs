...
namespace Tmds.Fuse
{
    class FuseMount : IFuseMount
    {
        ...
        private void Init(IntPtr ptr, IntPtr ptr2) => _mountTaskCompletion.TrySetResult(null);
        ...
        {
            ReadOnlySpan<byte> span = new ReadOnlySpan<byte>(path, int.MaxValue);
                ...
                try
                {
                    _fuseLoopTask = Task.Factory.StartNew(() =>
                    {
                        ...
                        try
                        {
                            ...
                            if (_mountOptions.Uid.HasValue)
                            {
                                LibFuse.fuse_opt_add_arg(&args, "-o");
                                LibFuse.fuse_opt_add_arg(&args, $"uid={_mountOptions.Uid.Value}");
                            }

                            if (_mountOptions.Gid.HasValue)
                            {
                                LibFuse.fuse_opt_add_arg(&args, "-o");
                                LibFuse.fuse_opt_add_arg(&args, $"gid={_mountOptions.Gid.Value}");
                            }

                            if (_mountOptions.AdditionalOptions != null)
                            {
                                foreach (string opt in _mountOptions.AdditionalOptions)
                                {
                                    LibFuse.fuse_opt_add_arg(&args, "-o");
                                    LibFuse.fuse_opt_add_arg(&args, opt);
                                }
                            }
        ...
        private Exception CreateException(string operation, int returnValue) => new FuseException($"Failed to {operation}, the function returned {returnValue}.");
        ...
        public unsafe void LazyUnmount()
        {
            lock (_gate)
            {
                if (_mounted)
                {
                    ...
                    // Give fuse a kick to help unblock the loop — access the mount point
                    // so fuse_loop sees an operation and can check its exit flag. Swallow
                    // any exception in case the directory was already removed by the OS or
                    // by fusermount3 during the unmount sequence.
                    try { File.GetLastAccessTime(_mountPoint); } catch { }
                }
            }
        }
        ...
        public void Dispose() => LazyUnmount();
    }
}