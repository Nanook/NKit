...
namespace CUETools.Codecs.Flake
{
    unsafe public class LpcSubframeInfo
    {
        ...
        internal double[,] autocorr_section_values;
        internal int[] autocorr_section_orders;
        //public int obits;
    ...
    unsafe public struct LpcWindowSection
    {
        ...
        unsafe public static void Detect(int _windowcount, float* window_segment, int stride, int sz, int bps, LpcWindowSection* sections)
        {
            ...
            SectionType[,] types = new SectionType[_windowcount, Lpc.MAX_LPC_SECTIONS * 2];
            int[,] alias = new int[_windowcount, Lpc.MAX_LPC_SECTIONS * 2];
            int[,] alias_set = new int[_windowcount, Lpc.MAX_LPC_SECTIONS * 2];
            for (int x = 0; x < sz; x++)
            {
    ...
    unsafe public class LpcContext
    {
        public LpcContext()
        {
            ...
            _reflection_coeffs = new double[Lpc.MAX_LPC_ORDER];
        ...
        {
            if (_autocorr_order > order)
                return;
            fixed (double* reff = _reflection_coeffs, autoc = autocorr_values, err = prediction_error)
            {
                for (int i = _autocorr_order; i <= order; i++) autoc[i] = 0;
                for (int section = 0; section < Lpc.MAX_LPC_SECTIONS; section++)
                {
                    ...
                    if (sections[section].m_id >= 0)
                    {
                        ...
                        for (int i = _autocorr_order; i <= order; i++)
                            autoc[i] += subframe.autocorr_section_values[sections[section].m_id, i];
                    }
                    else
                    {
                        sections[section].compute_autocorr(samples, window, _autocorr_order, order, blocksize, autoc);
                    }
                }
                Lpc.compute_schur_reflection(autoc, (uint)order, reff, err);
                _autocorr_order = order + 1;
            }
        }
#if XXX
        ...
        public double Akaike(int blocksize, int order, double alpha, double beta) =>
            ...
            (blocksize * Math.Log(prediction_error[order - 1])) + (Math.Log(blocksize) * order * (alpha + (beta * order)));
        ...
        /// </summary>
        /// <param name="lpcs">LPC coefficients CiBuffer (for all orders)</param>
        public void ComputeLPC(float* lpcs)
        {
            fixed (double* reff = _reflection_coeffs)
                Lpc.compute_lpc_coefs((uint)_autocorr_order - 1, reff, lpcs);
        }

        internal double[] autocorr_values;
        private double[] _reflection_coeffs;
        internal double[] prediction_error;
        internal int[] best_orders;
        internal int[] coefs;
        int _autocorr_order;
        internal int shift;

        public double[] Reflection => _reflection_coeffs;

        internal uint[] done_lpcs;
    }
}
