...
namespace CUETools.Codecs.Flake
{
    internal static class Crc32
    {
        ...
        public static uint ComputeChecksum(uint crc, byte val) => (crc >> 8) ^ table[(crc & 0xff) ^ val];
        ...
        const uint _UPolynomial = 0x04c11db7;
        const uint _UReversePolynomial = 0xedb88320;
        const uint _UReversePolynomial2 = 0xdb710641;

        private static readonly uint[,] _CombineTable;
        private static readonly uint[,] _SubstractTable;
        ...
        static unsafe Crc32()
        {
            ...
            for (uint i = 0; i < table.Length; i++)
            {
                ...
                for (int j = 0; j < 8; j++)
                    table[i] = (table[i] << 1) ^ ((table[i] & (1U << 31)) == 0 ? 0 : _UPolynomial);
                table[i] = Reflect(table[i], 32);
            }
            _CombineTable = new uint[_GF2_DIM, _GF2_DIM];
            _SubstractTable = new uint[_GF2_DIM, _GF2_DIM];
            _CombineTable[0, 0] = _UReversePolynomial;
            _SubstractTable[0, 31] = _UReversePolynomial2;
            for (int n = 1; n < _GF2_DIM; n++)
            {
                _CombineTable[0, n] = 1U << (n - 1);
                _SubstractTable[0, n - 1] = 1U << n;
            }
            fixed (uint* ct = &_CombineTable[0, 0], st = &_SubstractTable[0, 0])
            {
        ...
        public static unsafe uint Combine(uint crc1, uint crc2, long len2)
        {
            ...
            fixed (uint* ct = &_CombineTable[0, 0])
            {
                int n = 3;
                do
                {
        ...
        public static unsafe uint Subtract(uint crc1, uint crc2, long len2)
        {
            ...
            fixed (uint* st = &_SubstractTable[0, 0])
            {
                int n = 3;
                do
                {
