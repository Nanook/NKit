...
namespace NKit.Tests.Full.Wiped
{
    public partial class WipedImage_Default_Tests : WipedImageTestsBase
    {
        ...
        [Fact]
        public void Pneaviberf_HFN_7z_Scan()
        {
            string fileName = @"Pneaviberf (HFN).7z";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Default"));
            string outFolderName = $"Default_Pneaviberf_HFN_7z_Scan_{Guid.NewGuid():N}";
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("Pneaviberf (HFN).nkit.yaml", r.FinalName);
            Assert.Null(r.ChkCompared);
            Assert.Null(r.InFileParts);
            Assert.NotNull(r.OutFileParts);
            Assert.Equal(1, r.OutFileParts.Length);
            Assert.Equal(0x6f377d0L, r.OutFileParts[0].Size);
            ...
            Assert.Equal("Pneaviberf (HFN)", r.OutFileParts[0].FileName);
            ...
            Assert.Equal(0x6f377d0L, t.StepFiles[0].Size);
            ...
            Assert.Equal("Pneaviberf (HFN)", t.StepFiles[0].FileName);
