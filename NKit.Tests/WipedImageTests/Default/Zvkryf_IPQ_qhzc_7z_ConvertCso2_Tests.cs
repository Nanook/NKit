...
namespace NKit.Tests.Full.Wiped
{
    ...
    {
        //Default   / Retail     / ISO       / 222MiB  / RockRidge
        [Fact]
        public void Zvkryf_IPQ_qhzc_7z_ConvertCso2()
        {
            string fileName = @"Zvkryf IPQ qhzc.7z";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Default"));
            string outFolderName = $"Default_Zvkryf_IPQ_qhzc_7z_ConvertCso2_{Guid.NewGuid():N}";
            ...
            Assert.Equal("Zvkryf IPQ qhzc.7z//Zvkryf IPQ qhzc.iso", f.FriendlyFullPath.Replace(f.BasePath, ""));
            Assert.Equal("Zvkryf IPQ qhzc", f.CleanName);
            Assert.Equal("Zvkryf IPQ qhzc", f.Name);
            ...
            Assert.Equal("Zvkryf IPQ qhzc.iso", f.ImageFiles[0].FileName);
            Assert.Equal("Zvkryf IPQ qhzc", f.ImageFiles[0].NameOnly);
            Assert.Equal(".iso", f.ImageFiles[0].Extension);
            Assert.Equal(0xdded800L, f.ImageFiles[0].Size);
            ...
            Assert.Equal("Zvkryf IPQ qhzc.7z", f.ArchiveFiles[0].FileName);
            Assert.Equal("Zvkryf IPQ qhzc", f.ArchiveFiles[0].NameOnly);
            Assert.Equal(".7z", f.ArchiveFiles[0].Extension);
            Assert.Equal(0x2160L, f.ArchiveFiles[0].Size);
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("Zvkryf IPQ qhzc.cso", r.FinalName);
            Assert.Null(r.ChkCompared);
            Assert.NotNull(r.InFileParts);
            Assert.Equal(1, r.InFileParts.Length);
            Assert.Equal(0xdded800L, r.InFileParts[0].Size);
            Assert.NotNull(r.InFileParts[0].Checksums.ToString(true, true));
            Assert.Equal("Crc32:64168195, Md5:C053BDEB9630E89C00A0442FE218BC63, Sha1:7B1899F9005F326B6F6088B17D32F55FF775EF98, XxHash:52994F97DFCC8703", r.InFileParts[0].Checksums.ToString(true, true));
            Assert.Null(r.InFileParts[0].FileName);
            Assert.NotNull(r.OutFileParts);
            Assert.Equal(1, r.OutFileParts.Length);
            Assert.Equal(0x2b5fa4L, r.OutFileParts[0].Size);
            Assert.NotNull(r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.Equal("Crc32:53133AAC", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("Zvkryf IPQ qhzc.cso~", r.OutFileParts[0].FileName);
            Assert.NotNull(r.ResultCrc);
            Assert.Equal(0x64168195U, r.ResultCrc.Value);
            Assert.NotNull(r.ResultSize);
            Assert.Equal(0xdded800L, r.ResultSize.Value);
            ...
            Assert.Equal(0xdded800L, t.Size);
            Assert.Equal(0x64168195U, t.CRC);
            ...
            Assert.Equal("Zvkryf IPQ qhzc", t.Name);
            ...
            Assert.Equal("Zvkryf IPQ qhzc.cso", t.OutFileName);
            Assert.Null(t.OutKeyFilePath);
            Assert.NotNull(t.OutScanFilePath);
            Assert.Equal(Path.Combine(basePath, "Zvkryf IPQ qhzc.nkit.yaml"), t.OutScanFilePath);
            ...
            Assert.Equal(0x2b5fa4L, t.StepFiles[0].Size);
            ...
            Assert.Equal("Crc32:53133AAC", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("Zvkryf IPQ qhzc.cso~", t.StepFiles[0].FileName);
            ...
            Assert.Equal("Zvkryf IPQ qhzc", t.Scan.Name);
            Assert.Equal(SystemType.Default, f.SystemType);
            Assert.Equal(0x64168195U, t.Scan.Crc);
            Assert.Equal(0x64168195U, t.Scan.CrcDecrypted);
            Assert.Equal(0xdded800L, t.Scan.Size);
            Assert.Equal(32, t.Scan.VirtualFsTotalFileCount);
            ...
            Assert.Equal("00DDED800", t.Scan.Properties["Size"].ToXmlValue());
            Assert.NotNull(t.Scan.Properties["CRC"].ToXmlValue());
            Assert.Equal("64168195", t.Scan.Properties["CRC"].ToXmlValue());
            ...
            Assert.Equal("BEDA9448", t.Scan.Areas[0].AreaInfo.Properties["HeaderCrc"].ToXmlValue());
            Assert.NotNull(t.Scan.Areas[0].AreaInfo.Properties["HeaderXxHash"].ToXmlValue());
            Assert.Equal("7BA687FC8F9278E1", t.Scan.Areas[0].AreaInfo.Properties["HeaderXxHash"].ToXmlValue());
            Assert.NotNull(t.Scan.Areas[0].AreaInfo.Properties["PvdSectorCount"].ToXmlValue());
            Assert.Equal("00001BBDB", t.Scan.Areas[0].AreaInfo.Properties["PvdSectorCount"].ToXmlValue());
            ...
            Assert.Equal(0x64168195U, t.Scan.Areas[0].Crc);
            Assert.Equal(0x64168195U, t.Scan.Areas[0].CrcDecrypted);
            Assert.Equal(0xdded800L, t.Scan.Areas[0].Size);
            Dictionary<FsType, int> t_ScanTypes0 = WipedImageTestsBase.GetIsoFsTypes(t.Scan.Areas[0]);
            Assert.Equal(2, t_ScanTypes0.Count);
            Assert.Equal(4, t_ScanTypes0[FsType.System]);
            Assert.Equal(20, t_ScanTypes0[FsType.Iso9660]);
