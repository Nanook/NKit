...
namespace NKit.Tests.Full.Wiped
{
    public partial class WipedImage_Default_Tests : WipedImageTestsBase
    {
        ...
        [Fact]
        public void Romeo_7z_Expand()
        {
            string fileName = @"Romeo.7z";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Default"));
            string outFolderName = $"Default_Romeo_7z_Expand_{Guid.NewGuid():N}";
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("Romeo.iso", r.FinalName);
            Assert.Null(r.ChkCompared);
            Assert.Null(r.InFileParts);
            Assert.NotNull(r.OutFileParts);
            Assert.Equal(1, r.OutFileParts.Length);
            Assert.Equal(0x96000L, r.OutFileParts[0].Size);
            Assert.NotNull(r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.Equal("Crc32:7FBE44C8", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("Romeo.iso~", r.OutFileParts[0].FileName);
            ...
            Assert.Equal("Romeo.iso", t.OutFileName);
            ...
            Assert.Equal(0x96000L, t.StepFiles[0].Size);
            ...
            Assert.Equal("Crc32:7FBE44C8", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("Romeo.iso~", t.StepFiles[0].FileName);
