...
namespace NKit.Tests.Full.Wiped
{
    ...
    {
        //Retail     / ISO       / 755MiB  / Jiso lzo 2048 - Regular Iso
        [Fact]
        public void _552_Sebz_Ehffvn_jvgu_Ybir_HX_Ra_Se_Qr_Rf_Vg_jso_Expand()
        {
            string fileName = @"552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).jso";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Psp"));
            string outFolderName = $"Psp__552_Sebz_Ehffvn_jvgu_Ybir_HX_Ra_Se_Qr_Rf_Vg_jso_Expand_{Guid.NewGuid():N}";
            ...
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).jso", f.FriendlyFullPath.Replace(f.BasePath, ""));
            ...
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).jso", f.ImageFiles[0].FileName);
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg)", f.ImageFiles[0].NameOnly);
            Assert.Equal(".jso", f.ImageFiles[0].Extension);
            Assert.Equal(0x763648L, f.ImageFiles[0].Size);
            ...
            Assert.Equal("Md5", string.Join('|', i.VerifyChecksums.Select(a => a.ToString())));
            ...
            Assert.Equal("Md5:ECF9763DB07F0EBDDB29C1847315C936", i.SrcParts[0].Checksums.ToString(true, true));
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).iso", r.FinalName);
            Assert.Equal("Md5", string.Join('|', r.ChkCompared.Select(a => a.ToString())));
            ...
            Assert.Equal("Crc32:B5302DB0, Md5:ECF9763DB07F0EBDDB29C1847315C936", r.InFileParts[0].Checksums.ToString(true, true));
            Assert.Null(r.InFileParts[0].FileName);
            Assert.NotNull(r.OutFileParts);
            Assert.Equal(1, r.OutFileParts.Length);
            Assert.Equal(0x2f358000L, r.OutFileParts[0].Size);
            Assert.NotNull(r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.Equal("Crc32:B5302DB0", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).iso~", r.OutFileParts[0].FileName);
            ...
            Assert.Equal("InChecksums [Md5]", r.VerifyType);
            ...
            // Task Result
            ////////////////////////////////////////
            Assert.Equal(ContainerType.Jso, t.ContainerType);
            ...
            Assert.Equal("InChecksums [Md5]", t.VerifyType);
            Assert.NotNull(t.VerifyChecksum);
            Assert.Equal("ECF9763DB07F0EBDDB29C1847315C936", t.VerifyChecksum);
            Assert.Null(t.DatMatch);
            Assert.Null(t.ErrorMsg);
            Assert.NotNull(t.OutFileName);
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).iso", t.OutFileName);
            ...
            Assert.Equal(0x2f358000L, t.StepFiles[0].Size);
            ...
            Assert.Equal("Crc32:B5302DB0", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("552 - Sebz Ehffvn jvgu Ybir (HX) (Ra,Se,Qr,Rf,Vg).iso~", t.StepFiles[0].FileName);
