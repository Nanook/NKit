...
namespace NKit.Tests.Full.Wiped
{
    public partial class WipedImage_Gamecube_Tests : WipedImageTestsBase
    {
        ...
        [Fact]
        public void Gbjre_bs_Qehntn_Gur_Wncna_rvz_Expand()
        {
            string fileName = @"Gbjre bs Qehntn, Gur (Wncna).rvz";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Gamecube"));
            string outFolderName = $"Gamecube_Gbjre_bs_Qehntn_Gur_Wncna_rvz_Expand_{Guid.NewGuid():N}";
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("Gbjre bs Qehntn, Gur (Wncna).iso", r.FinalName);
            ...
            Assert.Equal("Crc32:DB5012A3", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("Gbjre bs Qehntn, Gur (Wncna).iso~", r.OutFileParts[0].FileName);
            ...
            Assert.Equal("CE9473838E0C54D1", t.VerifyChecksum);
            Assert.Null(t.DatMatch);
            Assert.Null(t.ErrorMsg);
            Assert.NotNull(t.OutFileName);
            Assert.Equal("Gbjre bs Qehntn, Gur (Wncna).iso", t.OutFileName);
            ...
            Assert.Equal("Crc32:DB5012A3", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("Gbjre bs Qehntn, Gur (Wncna).iso~", t.StepFiles[0].FileName);
