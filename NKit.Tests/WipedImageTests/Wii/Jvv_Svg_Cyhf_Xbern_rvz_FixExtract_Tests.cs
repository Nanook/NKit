...
namespace NKit.Tests.Full.Wiped
{
    public partial class WipedImage_Wii_Tests : WipedImageTestsBase
    {
        ...
        [Fact]
        public void Jvv_Svg_Cyhf_Xbern_rvz_FixExtract()
        {
            string fileName = @"Jvv Svg Cyhf (Xbern).rvz";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Wii"));
            string outFolderName = $"Wii_Jvv_Svg_Cyhf_Xbern_rvz_FixExtract_{Guid.NewGuid():N}";
            ...
            Assert.Equal(0x6398000L, r.OutFileParts[0].Size);
            Assert.NotNull(r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.Equal("Crc32:67BE8F6A", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("D1428D4DDBB25A56CEFB251EA92D36235B7FADC7_K_2A59DBB3~", r.OutFileParts[0].FileName);
            Assert.Equal(0x1890000L, r.OutFileParts[1].Size);
            Assert.NotNull(r.OutFileParts[1].Checksums.ToString(true, true));
            Assert.Equal("Crc32:9247C035", r.OutFileParts[1].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[1].FileName);
            Assert.Equal("ESCX560000_01__VAF_K_9247C035~", r.OutFileParts[1].FileName);
            ...
            Assert.Equal("D1428D4DDBB25A56CEFB251EA92D36235B7FADC7_K_2A59DBB3|ESCX560000_01__VAF_K_9247C035", t.OutFileName);
            ...
            Assert.Equal(0x6398000L, t.StepFiles[0].Size);
            ...
            Assert.Equal("Crc32:67BE8F6A", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("D1428D4DDBB25A56CEFB251EA92D36235B7FADC7_K_2A59DBB3~", t.StepFiles[0].FileName);
            Assert.Equal(0x1890000L, t.StepFiles[1].Size);
            ...
            Assert.Equal("Crc32:9247C035", t.StepFiles[1].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[1].FileName);
            Assert.Equal("ESCX560000_01__VAF_K_9247C035~", t.StepFiles[1].FileName);
