...
namespace NKit.Tests.Full.Wiped
{
    public partial class WipedImage_Wii_Tests : WipedImageTestsBase
    {
        ...
        [Fact]
        public void Uneirfg_Zbba_Gerr_bs_Genadhvyvgl_HFN_rvz_Expand()
        {
            string fileName = @"Uneirfg Zbba - Gerr bs Genadhvyvgl (HFN).rvz";
            string inPath = Path.GetFullPath(Path.Combine(@"../../../../../WipedImages", "Wii"));
            string outFolderName = $"Wii_Uneirfg_Zbba_Gerr_bs_Genadhvyvgl_HFN_rvz_Expand_{Guid.NewGuid():N}";
            ...
            ////////////////////////////////////////
            Assert.NotNull(r);
            Assert.Equal("Uneirfg Zbba - Gerr bs Genadhvyvgl (HFN).iso", r.FinalName);
            ...
            Assert.Equal("Crc32:DD35F2E6", r.OutFileParts[0].Checksums.ToString(true, true));
            Assert.NotNull(r.OutFileParts[0].FileName);
            Assert.Equal("Uneirfg Zbba - Gerr bs Genadhvyvgl (HFN).iso~", r.OutFileParts[0].FileName);
            ...
            Assert.Equal("BCE2569E6B8001B6", t.VerifyChecksum);
            Assert.Null(t.DatMatch);
            Assert.Null(t.ErrorMsg);
            Assert.NotNull(t.OutFileName);
            Assert.Equal("Uneirfg Zbba - Gerr bs Genadhvyvgl (HFN).iso", t.OutFileName);
            ...
            Assert.Equal("Crc32:DD35F2E6", t.StepFiles[0].Checksums.ToString(true, true));
            Assert.NotNull(t.StepFiles[0].FileName);
            Assert.Equal("Uneirfg Zbba - Gerr bs Genadhvyvgl (HFN).iso~", t.StepFiles[0].FileName);
